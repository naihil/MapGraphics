    ...
    : QWidget(parent)
{
    ...
    connect(renderTimer, &QTimer::timeout, this, &MapGraphicsView::renderTiles);
...
{
    return this->mapToScene(QPoint(this->width()/2,this->height()/2));
}
...
void MapGraphicsView::centerOn(const QPointF &pos)
{
    ...
    const QPointF qgsPos = _tileSource->ll2qgs(pos,this->zoomLevel());
...
QPointF MapGraphicsView::mapToScene(const QPoint &viewPos) const
{
    ...
    const QPointF qgsScenePos = _childView->mapToScene(viewPos);
...
void MapGraphicsView::setScene(MapGraphicsScene *scene)
{
    ...
    */
    ...
    // TODO: Check this private slot
    connect(this,
            SIGNAL(zoomLevelChanged(quint8)),
            childScene,
            SLOT(handleZoomLevelChanged()));
//    connect(this, &MapGraphicsView::zoomLevelChanged, childScene, &PrivateQGraphicsScene::handleZoomLevelChanged);
    ...
    connect(childView, &PrivateQGraphicsView::hadMouseDoubleClickEvent, this, &MapGraphicsView::handleChildMouseDoubleClick);
    connect(childView, &PrivateQGraphicsView::hadMouseMoveEvent, this, &MapGraphicsView::handleChildMouseMove);
    connect(childView, &PrivateQGraphicsView::hadMousePressEvent, this, &MapGraphicsView::handleChildMousePress);
    connect(childView, &PrivateQGraphicsView::hadMouseReleaseEvent, this, &MapGraphicsView::handleChildMouseRelease);
    connect(childView, &PrivateQGraphicsView::hadWheelEvent, this, &MapGraphicsView::handleChildViewScrollWheel);
    connect(childView, &PrivateQGraphicsView::hadContextMenuEvent, this, &MapGraphicsView::handleChildViewContextMenu);
...
void MapGraphicsView::setTileSource(QSharedPointer<MapTileSource> tSource)
{
    ...
    if (!_tileSource.isNull()) {
        ...
        connect(_tileSource.data(), &MapTileSource::destroyed, tileSourceThread, &QThread::quit);
        connect(tileSourceThread, &QThread::finished, tileSourceThread, &QThread::deleteLater);
    }
...
void MapGraphicsView::doTileLayout()
{
    ...
    for (qint32 x = xc; x < xMax; x++) {
        for (qint32 y = yc; y < yMax; y++) {
            ...
            const bool tileIsThere = placesWhereTilesAre.contains(scenePos);
            if (tileIsThere) {
                continue;
            }
